 ...
 */
class UL_WC_Widget_Price_Filter extends WC_Widget {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce ul_widget_price_filter';
		$this->widget_description = __( 'Отображать слайдер для фильтрации товаров по цене. Кастомный.', 'ul' );
		$this->widget_id          = 'ul_woocommerce_price_filter';
		$this->widget_name        = __( 'Фильтр по цене', 'ul' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Фильтр по цене', 'ul' ),
		...
		echo '<form method="get" action="' . esc_url( $form_action ) . '">
			<div class="price_slider_wrapper">
				...
				<div class="price_slider_amount">
					<input type="text" id="min_price" name="min_price" value="' . esc_attr( $min_price ) . '" data-min="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_min_amount', $min ) ) . '" placeholder="' . esc_attr__( 'Min price', 'ul' ) . '" />
					<input type="text" id="max_price" name="max_price" value="' . esc_attr( $max_price ) . '" data-max="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_max_amount', $max ) ) . '" placeholder="' . esc_attr__( 'Max price', 'ul' ) . '" />
					<div class="price_label" style="display:none;">
						' . esc_html__( 'Price:', 'ul' ) . ' <span class="from"></span> &mdash; <span class="to"></span>
					</div>
